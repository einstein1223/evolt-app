...
const selectedStation = computed(() => {
    const stations = Array.isArray(props.dbStations) ? props.dbStations : [];
    return stations.find(s => s.id === selectedStationId.value) || null;
});
...
const getNaiveBayesVisual = (hour) => {
    if (hour >= 17 && hour <= 20) return { label: 'Ramai', color: 'text-red-600', bg: 'bg-red-50', border: 'border-l-4 border-red-500' };
    if (hour >= 12 && hour <= 13) return { label: 'Sedang', color: 'text-orange-500', bg: 'bg-orange-50', border: 'border-l-4 border-orange-400' };
    return { label: 'Sepi', color: 'text-green-600', bg: 'bg-green-50', border: 'border-l-4 border-green-500' };
};
...
const timeOptions = computed(() => {
    ...
    for (let h = 0; h < 24; h++) {
        for (let m of [0, 30]) {
            ...
            let statusText = ''; let statusClass = ''; let containerClass = '';

            if (slotStart < currentMinutes - 15) { isDisabled = true; statusText = '(Lewat)'; statusClass = 'text-gray-400 text-xs italic'; }
            ...
            if (!isDisabled && selectedPort.value) {
                ...
                if (isClash) { isDisabled = true; statusText = '(Penuh)'; statusClass = 'text-red-600 font-bold text-xs'; containerClass = 'bg-gray-100 opacity-70'; }
            }

            if (!isDisabled) {
                const prediction = getNaiveBayesVisual(h);
                statusText = prediction.label; statusClass = `${prediction.color} text-[10px] font-bold uppercase tracking-wider ml-auto`; containerClass = `${prediction.bg} ${prediction.border}`;
            }

            options.push({ label: timeStr, value: timeStr, disabled: isDisabled, status: statusText, statusClass: statusClass, containerClass: containerClass });
        }
    }
    if (selectedStartTime.value && !options.find(o => o.value === selectedStartTime.value)) {
        options.push({ label: selectedStartTime.value, value: selectedStartTime.value, disabled: false, status: '', statusClass: '', containerClass: '' });
    }
    options.sort((a, b) => {
...
const handleBodyScroll = () => {
    ...
    document.body.classList.toggle('modal-open', isMobile() && anyModalOpen);
};

watch([showConfirmationModal, showQrisPaymentModal], ([confirmationNew, qrisNew]) => {
    handleBodyScroll();
    if (confirmationNew || qrisNew) {
...
const domicileCoordinates = computed(() => ({
    ...
    'Lubuk Baja': { lat: 1.1352, lng: 104.0108 },
    'Batu Aji': { lat: 1.0558, lng: 103.9781 },
    'Tiban': { lat: 1.1118, lng: 103.9902 },
    'Kabil': { lat: 1.0927, lng: 104.1293 },
    'Batu Ampar': { lat: 1.1668, lng: 104.0089 }
}));
        ...
        (position) => {
            ...
            if (map && window.L) {
                ...
                const userIcon = window.L.divIcon({
                    ...
                    html: `<div class="flex items-center justify-center w-8 h-8"><div class="w-5 h-5 bg-blue-600 rounded-full border-[3px] border-white shadow-lg animate-pulse"></div></div>`,
                    iconSize: [32, 32], iconAnchor: [16, 16] 
                });
                userMarker = window.L.marker([userLocation.value.lat, userLocation.value.lng], { icon: userIcon })
                    .addTo(map).bindPopup("<b class='text-blue-600'>Lokasi Anda</b>").openPopup();
            }
        },
        (error) => { isLocating.value = false; console.error(error); },
        { enableHighAccuracy: true }
    );
};

// FILTER STASIUN YANG VALID (Status != Tutup)
const validStations = computed(() => {
    const stations = Array.isArray(props.dbStations) ? props.dbStations : [];
    return stations.filter(s => {
        const status = s.status ? String(s.status).trim().toLowerCase() : '';
        return status !== 'tutup';
    });
});

const recommendedStations = computed(() => {
    let result = validStations.value.map(s => ({ ...s }));
    ...
    else if (formState.value.domicile && domicileCoordinates.value[formState.value.domicile]) center = domicileCoordinates.value[formState.value.domicile];
    ...
    result = result.map(station => {
        ...
        return { ...station, realDistance: dist, distance: isNaN(dist) ? 'N/A' : dist + ' km' };
    });
...
// WATCHER UNTUK FILTER DOMISILI
watch(() => formState.value.domicile, (newVal) => {
    if (newVal && domicileCoordinates.value[newVal] && map) {
        const coords = domicileCoordinates.value[newVal];
...
const getMarkerColor = (station) => {
    if (station.is_recommended) return '#7C3AED';
...
const renderMarkers = (L) => {
    ...
    validStations.value.forEach(s => {
        if (s.lat && s.lng) {
            ...
            const recommendationBadge = s.is_recommended ? `<div class="mb-2"><span class="bg-purple-100 text-purple-700 text-[10px] font-bold px-2 py-0.5 rounded border border-purple-200">⭐ ${s.recommendation_status || 'Recommended'}</span></div>` : '';
            ...
            marker.bindPopup(`
                <div class="font-sans text-left w-[200px] sm:w-[220px]">
                    <div class="p-3">
                        ...
                        <p class="text-xs text-gray-500 mb-3 flex items-start gap-1"><i class="fas fa-map-marker-alt mt-0.5 text-gray-400 shrink-0"></i><span class="line-clamp-2 leading-relaxed">${s.location}</span></p>
                        <div class="flex gap-2">
                            <a href="${googleMapsUrl}" target="_blank" class="flex-1 flex items-center justify-center gap-1 py-2 px-2 bg-white border border-gray-200 text-gray-600 text-xs font-bold rounded-lg hover:bg-gray-50 transition no-underline"><i class="fas fa-location-arrow"></i> Rute</a>
                            <button onclick="document.dispatchEvent(new CustomEvent('book-station', {detail: ${s.id}}))" class="flex-1 flex items-center justify-center gap-1 py-2 px-2 ${btnColor} text-white text-xs font-bold rounded-lg shadow-sm hover:shadow-md hover:opacity-90 transition transform active:scale-95"><i class="fas fa-bolt"></i> Pesan</button>
                        </div>
                    </div>
                </div>
            `);
...
watch(() => props.dbStations, () => {
    if (window.L) renderMarkers(window.L);
}, { deep: true });
...
const reserveStation = (id) => {
    ...
    if (selectedStation.value) {
        ...
        const now = new Date();
        selectedStartTime.value = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;
        if(availablePorts.value.length > 0) selectedPort.value = availablePorts.value[0].value;
...
onMounted(async () => {
    ...
    // --- 1. SET FILTER JIKA ADA ---
    if (params.has('domicile')) formState.value.domicile = params.get('domicile');
    if (params.has('station')) formState.value.station = params.get('station'); // Baca nama station dari URL

    // --- 2. CEK AUTO BOOKING (DARI KLIK REKOMENDASI) ---
    if (params.has('auto_book_id')) {
        const autoId = Number(params.get('auto_book_id'));
        // Beri sedikit delay agar data station computed property siap
        setTimeout(() => {
            selectStationFromMap(autoId); // Set selection
            reserveStation(autoId); // Buka modal
        }, 800);
    }
    ...
    try {
        ...
        // Prioritize centering on Domicile filter if exists
        if(formState.value.domicile && domicileCoordinates.value[formState.value.domicile]){
            centerMap = [domicileCoordinates.value[formState.value.domicile].lat, domicileCoordinates.value[formState.value.domicile].lng];
        }
        ...
        pollingInterval = setInterval(() => {
            router.reload({ only: ['dbStations'], preserveScroll: true, preserveState: true });
        }, 5000);
...
const estimatedDurationMinutes = computed(() => { 
    ...
    return Math.max(15, Math.round((Number(selectedDuration.value) / parsePowerValue(selectedStation.value.power)) * 60 * 1.1)); 
});
...
const priceBreakdown = computed(() => {
    ...
    if (selectedStation.value.is_private) { 
        unitPrice = basePrice; 
        totalPriceEnergy = unitPrice * Number(selectedDuration.value); 
    } else { 
        unitPrice = Math.round(basePrice / 30); 
        totalPriceEnergy = unitPrice * (Number(selectedDuration.value) || 1); 
    }
    ...
    return { 
        ...
        price: totalPriceEnergy, 
        service: serviceFee, 
        ppn, 
        total: totalPriceEnergy + serviceFee + ppn 
    };
});
...
const submitSearch = () => {
    if(formState.value.domicile && domicileCoordinates.value[formState.value.domicile] && map){
        const coords = domicileCoordinates.value[formState.value.domicile];
        map.flyTo([coords.lat, coords.lng], 13);
    }
    closeModal();
};
...
const onTouchEnd = () => { 
    ...
    if (dragOffset.value > 100) { 
        if (showSearchModal.value) closeModal(); 
        else cancelProcess(); 
    } 
    dragOffset.value = 0; 
};
    ...
    <div class="min-h-screen flex flex-col bg-gray-50 font-sans text-gray-800">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div class="flex justify-between items-center mb-4">
                    <div class="bg-white px-4 py-2 rounded-full shadow-sm border border-gray-100 flex items-center gap-2">
                        <div class="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div><span class="text-sm font-bold text-gray-700 font-mono">{{ formattedTime }}</span>
                    </div>
                </div>
                
                <div class="flex flex-col md:flex-row justify-end items-start md:items-center mb-6">
                    <div class="relative w-full md:w-72 group">
                        <input type="text" placeholder="Filter Lokasi..." class="w-full p-3.5 pl-12 border border-gray-200 rounded-2xl bg-white shadow-sm focus:ring-2 focus:ring-[#00C853] cursor-pointer" @click="openModal" readonly :value="formState.domicile ? `Area: ${formState.domicile}` : 'Cari Stasiun...'">
                        <i class="fas fa-search absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 group-hover:text-[#00C853]"></i>
                    </div>
                </div>

                <div class="relative w-full mb-8 rounded-3xl shadow-xl overflow-hidden border border-white h-72 md:h-[450px] bg-[#e9f5ff] z-0 group">
                    <div id="map" class="w-full h-full z-0"></div>
                    <button @click="getUserLocation" class="absolute bottom-4 right-4 z-[400] bg-white p-3 rounded-full shadow-lg border border-gray-200 text-gray-600 hover:text-blue-600 hover:bg-blue-50 transition transform active:scale-95 flex items-center justify-center w-10 h-10 md:w-12 md:h-12" title="Lokasi Saya">
                        <i v-if="!isLocating" class="fas fa-crosshairs text-lg"></i><i v-else class="fas fa-spinner fa-spin text-lg text-blue-500"></i>
                    </button>
                    <div v-if="userLocation" class="absolute top-4 left-1/2 transform -translate-x-1/2 z-[400] bg-blue-500/90 backdrop-blur-sm text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-md flex items-center gap-2">
                        <div class="w-2 h-2 bg-white rounded-full animate-pulse"></div>GPS Aktif
                    </div>
                </div>

                <div id="stations-list-container">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-xl sm:text-2xl font-bold text-gray-800">Stasiun <span class="text-[#00C853]">Terdekat</span></h2>
                        <span class="text-xs sm:text-sm font-medium text-gray-500 bg-white px-3 py-1 rounded-full border border-gray-200 shadow-sm">{{ nearestStations.length }} Ditemukan</span>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
                        <div v-for="station in nearestStations" :key="station.id" :id="`station-card-${station.id}`"
                            ...
                            :class="{ 'ring-4 ring-[#00C853] ring-opacity-50 bg-green-50 transform -translate-y-2 order-first': selectedStationId === station.id }">
                            
                            <div v-if="station.is_recommended" class="absolute top-4 left-4 bg-purple-100 text-purple-700 text-[10px] font-bold px-2 py-1 rounded-full border border-purple-200 flex items-center gap-1 z-10 animate-pulse">
                                <i class="fas fa-star"></i> {{ station.recommendation_status || 'Rekomendasi AI' }}
                            </div>

                            <div v-if="station.is_private" class="absolute top-4 right-4 bg-orange-100 text-orange-700 text-[10px] font-bold px-2 py-1 rounded-full border border-orange-200 flex items-center gap-1 z-10">
                                <i class="fas fa-home"></i> Mitra Host
                            </div>

                            <div class="flex justify-between items-start mb-3 mt-4">
                                <div>
                                    <h2 class="text-lg sm:text-xl font-bold text-gray-900 leading-tight mb-1 pr-16">{{ station.name }}</h2>
                                    <div class="flex items-center text-xs sm:text-sm text-gray-500">
                                        <i class="fas fa-map-marker-alt mr-1.5 text-gray-400"></i><span class="font-bold text-[#00C853]">{{ station.distance }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-auto">
                                <button @click="reserveStation(station.id)" :disabled="!station.isBookable"
                                    ...
                                    <i class="fas fa-ticket-alt"></i> <span>{{ station.isBookable ? 'Pesan Sekarang' : 'Penuh' }}</span>
                                </button>
                            </div>
                        </div>
                    </div>
        ...
        <Transition name="slide-up">
            <div v-if="showSearchModal" class="fixed inset-0 bg-gray-900/80 backdrop-blur-sm flex items-end sm:items-center justify-center z-[99999] p-0 sm:p-4" @click.self="closeModal">
                <div class="bg-white w-full h-auto sm:max-w-2xl rounded-t-[2rem] sm:rounded-3xl p-6 sm:p-8 shadow-2xl flex flex-col relative transition-transform duration-300"
                    ...
                    <form @submit.prevent="submitSearch" class="space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="relative dropdown-container"><label class="block text-xs font-bold text-gray-500 uppercase mb-1.5">Domisili</label>
                                <div @click="toggleDropdown('domicile')" class="dropdown-trigger" :class="{ 'active': activeDropdown === 'domicile' }"><span class="truncate">{{ formState.domicile || 'Pilih Lokasi' }}</span><i class="fas fa-chevron-down text-xs transition-transform" :class="{ 'rotate-180': activeDropdown === 'domicile' }"></i></div>
                                <div v-if="activeDropdown === 'domicile'" class="dropdown-menu">
                                    <div v-for="opt in Object.keys(domicileCoordinates)" :key="opt" @click="selectOption('domicile', opt)" class="dropdown-item" :class="{ 'selected': formState.domicile === opt }">{{ opt }}</div>
                                </div>
                            </div>
                        </div>
        ...
        <Transition name="slide-up">
             <div v-if="showConfirmationModal && selectedStation" class="fixed inset-0 bg-gray-900/80 backdrop-blur-sm flex items-end sm:items-center justify-center z-[99999] p-0 md:p-8" @click.self="cancelProcess">
                <div class="bg-white w-full max-h-[90vh] flex flex-col rounded-t-[2rem] sm:rounded-3xl shadow-2xl md:max-w-4xl relative transition-transform duration-300"
                    :style="{ transform: `translateY(${dragOffset}px)` }"
                    @touchstart="onTouchStart" @touchmove="onTouchMove" @touchend="onTouchEnd">
                    
                    <div class="flex-none pt-4 pb-2 sm:hidden" @click="cancelProcess">
                        <div class="w-16 h-1.5 bg-gray-300 rounded-full mx-auto"></div>
                    </div>

                    <div class="flex-none px-6 pt-2 pb-2 sm:px-8 sm:pt-8"><h3 class="text-xl font-bold text-gray-900 text-center mt-4 sm:mt-0">Konfirmasi Pesanan</h3></div>
                    <div class="flex-1 overflow-y-auto px-6 sm:px-8 custom-scrollbar pb-4">
                        <div class="space-y-4 mt-4">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                                <div class="relative dropdown-container"><label class="block text-xs font-bold text-gray-500 uppercase mb-1.5">Waktu Mulai</label>
                                    <div @click="toggleDropdown('time')" class="dropdown-trigger" :class="{ 'active': activeDropdown === 'time' }"><span class="text-gray-800 truncate flex items-center gap-2"><i class="fas fa-clock text-gray-400"></i> {{ displayTime }}</span><i class="fas fa-chevron-down text-gray-400 text-xs transition-transform" :class="{ 'rotate-180': activeDropdown === 'time' }"></i></div>
                                    <div v-if="activeDropdown === 'time'" class="dropdown-menu max-h-60">
                                        <div v-for="time in timeOptions" :key="time.value" @click="!time.disabled && selectOption('time', time.value)" class="dropdown-item flex justify-between items-center border-b border-gray-50 last:border-0" :class="[time.containerClass,{'selected ring-2 ring-inset ring-[#00C853]': selectedStartTime === time.value, 'opacity-50 cursor-not-allowed bg-gray-100': time.disabled}]">
                                            <span class="font-mono text-sm" :class="{'text-gray-400': time.disabled, 'text-gray-800': !time.disabled}">{{ time.value }}</span><span v-if="time.status" :class="time.statusClass">{{ time.status }}</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="relative dropdown-container"><label class="block text-xs font-bold text-gray-500 uppercase mb-1.5">Port Charging</label>
                                    <div @click="toggleDropdown('port')" class="dropdown-trigger" :class="{ 'active': activeDropdown === 'port' }"><span class="text-gray-800 truncate">{{selectedPort ? portOptions.find(p => p.value === selectedPort)?.label : 'Pilih Port'}}</span><i class="fas fa-chevron-down text-gray-400 text-xs transition-transform" :class="{ 'rotate-180': activeDropdown === 'port' }"></i></div>
                                ...
                                <div class="relative dropdown-container"><label class="block text-xs font-bold text-gray-500 uppercase mb-1.5">{{ selectedStation.is_private ? 'Paket Durasi' : 'Target Energi (kWh)' }}</label>
                                    <div @click="toggleDropdown('duration')" class="dropdown-trigger" :class="{ 'active': activeDropdown === 'duration' }"><span class="text-gray-800 truncate">{{durationOptions.find(d => d.value === selectedDuration)?.label || 'Pilih Paket'}}</span><i class="fas fa-chevron-down text-gray-400 text-xs transition-transform" :class="{ 'rotate-180': activeDropdown === 'duration' }"></i></div>
                                    <div v-if="activeDropdown === 'duration'" class="dropdown-menu">
                                        <div v-for="duration in durationOptions" :key="duration.value" @click="selectOption('duration', duration.value)" class="dropdown-item flex flex-col items-start" :class="{ 'selected': selectedDuration === duration.value }"><span class="font-bold">{{ duration.label }}</span><span v-if="duration.desc" class="text-[10px] text-gray-500">{{ duration.desc }}</span></div>
                                    </div>
                                </div>
                            </div>
                             <div class="bg-gray-50 rounded-2xl p-4 space-y-2 border border-gray-100">
                                ...
                                <div class="flex justify-between items-center text-sm">
                                    <span class="text-gray-500">{{ selectedStation.is_private ? 'Harga per Jam' : 'Harga per kWh' }}</span>
                                    <span class="font-medium text-gray-800">{{ formatRupiah(priceBreakdown.pricePerUnit) }}</span>
                                </div>
                                <div class="flex justify-between items-center text-sm">
                                    <span class="text-gray-600">{{ selectedStation.is_private ? `Sewa (${selectedDuration} Jam)` : `Energi (${selectedDuration} kWh)` }}</span>
                                    <span class="font-medium text-gray-800">{{ formatRupiah(priceBreakdown.price) }}</span>
                                </div>
                                <div class="flex justify-between items-center text-sm">
                                    <span class="text-gray-600">Biaya Layanan (Admin)</span>
                                    <span class="font-medium text-gray-800">{{ formatRupiah(priceBreakdown.service) }}</span>
                                </div>
                                <div class="flex justify-between items-center text-sm">
                                    <span class="text-gray-600">PPN (11%)</span>
                                    <span class="font-medium text-gray-800">{{ formatRupiah(priceBreakdown.ppn) }}</span>
                                </div>
                                <div class="flex justify-between items-center pt-3 border-t border-gray-200 mt-2">
                                    ...
                                    <span class="font-extrabold text-xl text-[#00C853]">{{ calculateTotalFormatted }}</span>
                                </div>
                            </div>
                        </div>
        ...
        <Transition name="fade">
            <div v-if="showQrisPaymentModal" class="fixed inset-0 bg-gray-900/90 backdrop-blur-sm flex items-center justify-center z-[99999] p-4" @click.self="cancelProcess">
                <div class="bg-white rounded-3xl p-8 shadow-2xl w-full max-w-md text-center relative overflow-hidden">
                    ...
                    <div class="bg-white p-2 rounded-xl shadow-inner border border-gray-100 inline-block mb-6 relative">
                        ...
                        <div v-if="isProcessingPayment" class="absolute inset-0 bg-white/90 flex flex-col items-center justify-center rounded-lg backdrop-blur-sm transition-opacity duration-300">
                            <div class="animate-spin rounded-full h-12 w-12 border-4 border-[#00C853] border-t-transparent mb-3"></div><span class="text-sm font-bold text-[#00C853] animate-pulse">Memverifikasi...</span>
                        </div>
                    </div>
        ...
        <Transition name="fade">
            <div v-if="showReceiptModal" class="fixed inset-0 bg-[#00C853] flex flex-col items-center justify-center z-[99999] p-4">
                 <div class="bg-white rounded-3xl w-full max-w-md shadow-2xl p-8 text-center relative overflow-hidden">
                    ...
                    <div v-if="!selectedStation.is_private">
                        ...
                        <div class="bg-gray-50 p-4 rounded-2xl border-2 border-dashed border-[#00C853] inline-block mb-6">
                            <img :src="`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=ACCESS_TOKEN_${transactionCode || selectedStation.bookingNumber}`" alt="QR Access" class="w-40 h-40 mix-blend-multiply">
                    ...
                    <div v-else class="bg-orange-50 p-5 rounded-2xl border-2 border-orange-200 text-left mb-6 relative overflow-hidden">
                        ...
                        <div class="text-center p-3 bg-white rounded-xl border border-orange-100 shadow-sm">
                            ...
                            <div class="text-2xl font-mono font-black text-gray-900 tracking-widest">{{ (transactionCode || selectedStation.bookingNumber).replace('BK-', '') }}</div>
                        </div>
                    </div>
                    ...
                    <button @click="router.visit('/status-charging')" class="w-full mt-3 py-3 bg-[#00C853] text-white font-bold rounded-xl hover:bg-[#00A142] shadow-lg transition flex items-center justify-center gap-2"><i class="fas fa-charging-station"></i> Lihat Status Charging</button>
                </div>
            </div>
        </Transition>
    </div>
...
:deep(.leaflet-popup-content-wrapper) { border-radius: 1rem; padding: 0; overflow: hidden; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04); }
:deep(.leaflet-popup-content) { margin: 0 !important; width: auto !important; }
:deep(.leaflet-popup-tip) { background: white; }
:deep(.leaflet-container a.leaflet-popup-close-button) { top: 8px; right: 8px; color: white; padding: 4px; width: 24px; height: 24px; border-radius: 50%; background: rgba(0, 0, 0, 0.2); backdrop-filter: blur(4px); display: flex; align-items: center; justify-content: center; font: 16px/14px Tahoma, Verdana, sans-serif; border: 1px solid rgba(255, 255, 255, 0.3); transition: all 0.2s; }
:deep(.leaflet-container a.leaflet-popup-close-button:hover) { color: white; background: rgba(0, 0, 0, 0.4); }
</style>