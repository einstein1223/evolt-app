...
onMounted(async () => {
    try {
        ...
        // Move zoom control to bottom-right to avoid navbar overlap
...
<template>
    <div class="min-h-screen bg-gray-100 font-sans text-gray-800">
        ...
        <main class="pt-24 py-10 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto space-y-10">

                <!-- HEADER -->
                <div class="flex justify-between items-center">
                    <h1 class="text-3xl font-bold text-gray-900">Dashboard Admin</h1>
                    <span class="px-4 py-1 bg-gray-200 rounded-full text-sm font-semibold text-gray-600">Mode: Realtime
                        Database</span>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div id="map" style="height: 400px; width: 100%;" class="rounded-xl bg-gray-100 z-10"></div>
                    </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div
                        class="p-6 flex flex-col md:flex-row justify-between items-center gap-4 border-b border-gray-100">
                        <div>
                        ...
                        <div class="flex gap-3 w-full md:w-auto">
                            <div class="relative flex-grow md:flex-grow-0">
                                <input v-model="searchQuery" type="text" placeholder="Cari stasiun..."
                                    class="pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm w-full focus:ring-[#00C853] focus:border-[#00C853]">
                            ...
                            <button @click="openAddStationModal"
                                class="bg-[#00C853] text-white px-5 py-2 rounded-lg text-sm font-bold hover:bg-[#00A142] shadow-md transition flex items-center gap-2 whitespace-nowrap">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-100">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Nama
                                        Stasiun</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Lokasi
                                    </th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Tipe
                                        Charger</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Jam Ops
                                    </th>
                                    <th class="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase">Aksi
                                    </th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-100">
                                <tr v-for="station in filteredStations" :key="station.id"
                                    ...
                                    <td class="px-6 py-4 space-x-1">
                                        <span v-for="(c, i) in (station.chargers_detail || [])" :key="i"
                                            :class="getChargerClass(c.type)">{{ c.type }}</span>
                                        <span v-if="!station.chargers_detail?.length"
                                            class="text-xs text-gray-400">-</span>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-600">{{ station.operationalHours || '24/7' }}
                                    </td>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="p-6 border-b border-gray-100">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <select v-model="selectedReportStation"
                                class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-[#00C853] focus:border-[#00C853]">
                                <option value="">Semua Stasiun</option>
                                <option v-for="s in reportStationOptions" :key="s" :value="s">{{ s }}</option>
                            </select>
                            <select v-model="selectedReportMonth"
                                class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-[#00C853] focus:border-[#00C853]">
                                <option value="">Semua Bulan</option>
                                <option v-for="m in reportMonthOptions" :key="m" :value="m">{{ m }}</option>
                            </select>
                            <select v-model="selectedReportYear"
                                class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-[#00C853] focus:border-[#00C853]">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-100">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Nama SPKLU
                                    </th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Domisili
                                    </th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Periode
                                    </th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Sesi
                                        (Total)</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Pendapatan
                                        Bersih</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                <tr v-for="report in filteredReports" :key="report.id" class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-600">{{ report.week }} ({{ report.month }} {{
                                        report.year }})</td>
                                    <td class="px-6 py-4 text-sm text-gray-600">{{ report.totalSessions }}</td>
                                    <td class="px-6 py-4 text-sm font-bold text-gray-900">{{
                                        formatRupiah(report.revenue) }}</td>
                                    <td class="px-6 py-4">
                                        <span
                                            :class="['px-2 py-1 text-xs font-bold rounded-full', report.status === 'Terkirim' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700']">
                                            {{ report.status }}
                                        </span>
                                    </td>
                                </tr>
                                <tr v-if="filteredReports.length === 0">
                                    <td colspan="6" class="px-6 py-8 text-center text-gray-500">Belum ada transaksi
                                        booking yang terekam.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-100">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Total
                                        Charging</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Terakhir
                                        Aktif</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">Status
                                    </th>
                                </tr>
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                <tr v-for="user in props.dbUserReports" :key="user.id" class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4">
                                        <span
                                            :class="['px-2 py-1 text-xs font-bold rounded-full', user.status === 'Aktif' ? 'bg-blue-100 text-blue-700' : user.status === 'Baru' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600']">
                                            {{ user.status }}
                                        </span>
                                    </td>
                                </tr>
                ...
                <form @submit.prevent="addStation" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input v-model="newStation.operationalHours" type="text" class="w-full border rounded p-2"
                                required>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input v-model="newStation.latitude" type="number" step="any"
                                class="w-full border rounded p-2" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium">Longitude</label>
                            <input v-model="newStation.longitude" type="number" step="any"
                                class="w-full border rounded p-2" required>
                        </div>
                    </div>
                    ...
                    <div class="border-t pt-4 mt-2">
                        ...
                        <div v-for="(charger, idx) in newStation.chargers" :key="idx"
                            class="bg-gray-50 p-3 rounded mb-2 border relative">
                            <div class="flex justify-between mb-2">
                                <span class="text-xs font-bold text-gray-500">Port {{ idx + 1 }}</span>
                                <button type="button" @click="removeChargerBlock(idx)"
                                    class="text-red-500 text-xs hover:underline"
                                    v-if="newStation.chargers.length > 1">Hapus</button>
                            </div>
                            <div class="grid grid-cols-3 gap-2">
                                ...
                                <div>
                                    ...
                                    <input v-model="charger.power" type="number"
                                        class="w-full border rounded p-1 text-sm">
                                </div>
                                <div>
                                    <label class="text-xs text-gray-500">Konektor</label>
                                    <input v-model="charger.connector" type="text" placeholder="Type 2"
                                        class="w-full border rounded p-1 text-sm">
                                </div>
                            </div>
                        </div>
                        <button type="button" @click="addChargerBlock"
                            class="text-[#00C853] text-sm font-bold hover:underline">+ Tambah Port Lain</button>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 mt-6 border-t pt-4">
                        ...
                        <button type="submit"
                            class="px-6 py-2 bg-[#00C853] text-white rounded font-bold hover:bg-[#00A142]">Simpan
                            Data</button>
                    </div>
                </form>
            </div>
