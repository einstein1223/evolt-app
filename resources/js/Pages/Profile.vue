...
<div class="min-h-screen bg-gray-50 font-poppins flex flex-col text-gray-800">
    ...
    <main class="flex-1 max-w-7xl mx-auto w-full p-4 sm:p-6 flex flex-col md:flex-row gap-8">
      ...
      <aside class="w-full md:w-72 flex-shrink-0">
        <div class="bg-white shadow-lg rounded-2xl p-6 sticky top-24 border border-gray-100">
            <div class="mb-8 text-center">
                ...
                <div v-if="form.nomor_plat" class="mt-3 inline-block px-3 py-1 bg-[#f0fdf4] text-[#16a34a] rounded-full text-xs font-bold border border-green-200">
                    {{ form.nomor_plat }}
                </div>
            </div>
      ...
      <section class="flex-1 bg-white shadow-xl rounded-2xl p-6 sm:p-10 min-h-[600px] border border-gray-100 relative overflow-hidden">
          ...
          <form @submit.prevent="submitProfile" class="space-y-6">
            ...
            <div v-if="activeTab === 'informasi_akun'" class="grid sm:grid-cols-2 gap-6 animate-slide-up">
                <div class="col-span-2">
                  ...
                  <input v-model="form.username" type="text" class="w-full border border-gray-300 rounded-xl p-3.5 focus:ring-2 focus:ring-[#00C853] focus:border-[#00C853] transition" />
                </div>
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1.5">Jenis Kelamin</label>
                  <select v-model="form.gender" class="w-full border border-gray-300 rounded-xl p-3.5 focus:ring-2 focus:ring-[#00C853] focus:border-[#00C853] bg-white">
                ...
                <div>
                  ...
                  <select v-model="form.city" class="w-full border border-gray-300 rounded-xl p-3.5 focus:ring-2 focus:ring-[#00C853] focus:border-[#00C853] bg-white">
                ...
                <div class="col-span-2 pt-4 flex justify-end">
                    <button type="submit" :disabled="form.processing" class="px-8 py-3.5 font-bold rounded-xl shadow-lg bg-[#00C853] text-white hover:bg-[#008e3b] transition transform active:scale-95 disabled:opacity-70">
                        Simpan Perubahan
                    </button>
                </div>
            </div>
            ...
            <div v-else-if="activeTab === 'informasi_personal'" class="space-y-6 animate-slide-up">
              <div class="grid sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input :value="form.email" disabled class="w-full border border-gray-200 rounded-xl p-3.5 bg-gray-100 text-gray-500 cursor-not-allowed" />
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Nomor HP / WhatsApp</label>
                    <input v-model="form.nomor_telepon" type="text" class="w-full border border-gray-300 rounded-xl p-3.5 focus:ring-2 focus:ring-[#00C853] focus:border-[#00C853]" />
                  </div>
              </div>
              ...
              <div class="bg-[#fcfdfa] border border-[#e3fbd8] rounded-2xl p-6 relative overflow-hidden">
                  ...
                  <div class="grid grid-cols-2 gap-4">
                      <div>
                          ...
                          <input v-model="form.car_brand" class="w-full bg-white border border-gray-200 rounded-lg p-2.5 text-gray-700 font-medium text-sm" readonly>
                      </div>
                      <div>
                          <label class="block text-xs font-bold text-gray-400 uppercase mb-1">Model</label>
                          <input v-model="form.car_series" class="w-full bg-white border border-gray-200 rounded-lg p-2.5 text-gray-700 font-medium text-sm" readonly>
                      </div>
                      <div>
                          <label class="block text-xs font-bold text-gray-400 uppercase mb-1">Tipe</label>
                          <input v-model="form.car_type" class="w-full bg-white border border-gray-200 rounded-lg p-2.5 text-gray-700 font-medium text-sm" readonly>
                      </div>
                      <div>
                          <label class="block text-xs font-bold text-gray-400 uppercase mb-1">Plat Nomor</label>
                          <input v-model="form.nomor_plat" class="w-full bg-white border border-gray-200 rounded-lg p-2.5 text-gray-900 font-bold text-sm uppercase focus:ring-2 focus:ring-[#00C853] focus:border-[#00C853]">
                      </div>
                  </div>
                  <p class="text-xs text-gray-400 mt-3 italic">*Hanya Plat Nomor yang dapat diubah langsung. Hubungi admin untuk ubah jenis mobil.</p>
              </div>
        ...
        <div v-else-if="activeMenu === 'orders'" class="animate-fade-in">
          ...
          <div v-if="props.orders && props.orders.length > 0" class="space-y-4">
             <div v-for="order in props.orders" :key="order.id" class="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col md:flex-row justify-between md:items-center gap-4 group">
                <div>
                    <div class="font-bold text-gray-800 text-lg group-hover:text-[#00C853] transition-colors">{{ order.station_name }}</div>
          ...
          <div v-else class="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-gray-200 rounded-3xl bg-gray-50">
              ...
              <Link :href="route('dashboard')" class="px-6 py-3 bg-[#00C853] text-white font-bold rounded-xl hover:bg-[#008e3b] transition shadow-lg active:scale-95">
                  Cari Stasiun
              </Link>
          </div>
        </div>
        ...
        <div v-else-if="activeMenu === 'password'" class="animate-fade-in">
          ...
          <form @submit.prevent="changePassword" class="space-y-5 max-w-lg">
            ...
            <div class="pt-2">
                <button type="submit" :disabled="passwordForm.processing" class="px-8 py-3.5 rounded-xl bg-[#00C853] text-white font-bold shadow-lg hover:bg-[#008e3b] transition transform active:scale-95 disabled:opacity-70">
